...
function updateContainerSize() {
  const svg = document.querySelector('svg');
  const maxHeight = Math.max(...Array.from(document.querySelectorAll('.node')).map(el => el.getBoundingClientRect().bottom));
...
function addChildNode(element) {
  ...
  input.placeholder = '_';
  input.maxLength = 1;
  input.oninput = function () { updateDisplayWord(this); };

  const addButton = document.createElement('button');
  addButton.className = 'add-btn';
  addButton.textContent = '+';
  addButton.onclick = function () { addChildNode(this); };

  const deleteButton = document.createElement('button', { class: 'delete-btn' });
...
function updateDisplayWord(node) {
  ...
  document.getElementById('wordsFormedCount').textContent = `Words Formed: ${wordsFormed.length}`;
